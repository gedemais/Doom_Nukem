...
//	printf("nb_meshs = %d\n", env->maps[0].nb_meshs);
	...
	while ((int)i < ((t_mesh*)dyacc(&env->maps[0].meshs, j))->nb_tris)
	{
		...
		t.points[0] = vec_add(t.points[0], (t_vec3d){0.0f, 0.0f, 10.0f});
		t.points[1] = vec_add(t.points[1], (t_vec3d){0.0f, 0.0f, 10.0f});
		t.points[2] = vec_add(t.points[2], (t_vec3d){0.0f, 0.0f, 10.0f});
